...
public class ParsingNewsUN {
    ...
    public int getArticles() {
        int articleCount = 0;
        String resourceKey = "newsun";
        String resourceFullName = "Новости ООН";
        String resourceShortName = "Новости ООН";
        String resourceLink = "https://news.un.org/ru/";
        String resourceNewsLink = "https://news.un.org/feed/subscribe/ru/news/all/rss.xml";
        ...
        try {
            ...
            for (int i = 0; i < articleCollectionCount; i++) {
                ...
                Elements articlesDescriptions = docArticleDescription.getElementsByClass("field field-name-field-news-topics field-type-entityreference field-label-hidden");
                if (articlesDescriptions != null) {
                    for (Element articleDescription : articlesDescriptions) {
                        if (articleDescription.select("a").text() != null) {
                            rubricAliasName = articleDescription.select("a").first().text();
                            articleRubricList.add(k++, new ArticleRubric().addRubricName(rubricAliasName, true, dateStamp));
                        }
                    }
                }
