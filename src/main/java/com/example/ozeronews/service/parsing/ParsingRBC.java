...
public class ParsingRBC {
    ...
    public int getArticles() {
        int articleCount = 0;
        String resourceKey = "rbc";
        String resourceFullName = "РБК";
        String resourceShortName = "РБК";
        String resourceLink = "https://www.rbc.ru";
        String resourceNewsLink = "http://static.feed.rbc.ru/rbc/logical/footer/news.rss";
        ...
        try {
            ...
            for (int i = 0; i < articleCollectionCount; i++) {
                ...
                Elements articleDescription = docArticleDescription.getElementsByClass("article");

                for (int j = 0; j < articleDescription.size(); j++) {
                    articleImage = articleDescription.get(j).select("img[class=article__main-image__image g-image js-rbcslider-image]").attr("src");

                    rubricAliasName = articleDescription.get(j).select("a[class=article__header__category]").text();
                    articleRubricList.add(k++, new ArticleRubric().addRubricName(rubricAliasName, true, dateStamp));
